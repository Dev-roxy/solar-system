export const planetsData = [
  {
    name: "Sun",
    radius: 5,
    distanceFromSun: 0,
    color: 0xffcc00,
    texture: "public/textures/sun.jpg",
    orbitSpeed: 0,
    rotationSpeed: 0.01,
    rings: [],
    moons: [],
  },
  {
    name: "Mercury",
    radius: 1,
    distanceFromSun: 10,
    color: 0xaaaaaa,
    texture: "public/textures/mercury.jpg",
    orbitSpeed: 0.00833,
    rotationSpeed: 0.01,
    rings: [],
    moons: [],
  },
  {
    name: "Venus",
    radius: 1.5,
    distanceFromSun: 15,
    color: 0xffddaa,
    texture: "public/textures/venus.jpg",
    orbitSpeed: 0.00322,
    rotationSpeed: 0.005,
    rings: [],
    moons: [],
  },
  {
    name: "Earth",
    radius: 2,
    distanceFromSun: 20,
    color: 0x2194ce,
    texture: "public/textures/earth.jpg",
    orbitSpeed: 0.002,
    rotationSpeed: 0.01,
    rings: [],
    moons: [
      {
        name: "Moon",
        radius: 0.3,
        distanceFromPlanet: 0.5,
        texture: "public/textures/moon.jpg",
        orbitSpeed: 0.0025,
        rotationSpeed: 0.01,
        isTidalyLocked: true, // Moon is tidally locked to Earth
      },
    ],
  },
  {
    name: "Mars",
    radius: 1.2,
    distanceFromSun: 26,
    color: 0xff3300,
    texture: "public/textures/mars.jpg",
    orbitSpeed: 0.00106,
    rotationSpeed: 0.008,
    rings: [],
    moons: [
      {
        name: "Phobos",
        radius: 0.2,
        distanceFromPlanet: 0.5,
        texture: "public/textures/phobos.jpg",
        orbitSpeed: 0.002,
        rotationSpeed: 0.01,
      },
      {
        name: "Deimos",
        radius: 0.15,
        distanceFromPlanet: 1,
        texture: "public/textures/deimos.jpg",
        orbitSpeed: 0.0015,
        rotationSpeed: 0.01,
      },
    ],
  },
  {
    name: "Jupiter",
    radius: 4,
    distanceFromSun: 35,
    color: 0xffa500,
    texture: "public/textures/jupiter.jpg",
    orbitSpeed: 0.0001686,
    rotationSpeed: 0.02,
    rings: [],
    moons: [
      {
        name: "Io",
        radius: 0.5,
        distanceFromPlanet: 1.5,
        texture: "public/textures/io.png",
        orbitSpeed: 0.0025,
        rotationSpeed: 0.01,
      },
      {
        name: "Europa",
        radius: 0.4,
        distanceFromPlanet: 2,
        texture: "public/textures/europa.jpg",
        orbitSpeed: 0.002,
        rotationSpeed: 0.01,
      },
      {
        name: "Ganymede",
        radius: 0.6,
        distanceFromPlanet: 2.5,
        texture: "public/textures/ganymede.jpg",
        orbitSpeed: 0.0018,
        rotationSpeed: 0.01,
      },
      {
        name: "Callisto",
        radius: 0.7,
        distanceFromPlanet: 3,
        texture: "public/textures/callisto.jpg",
        orbitSpeed: 0.0015,
        rotationSpeed: 0.01,
      },
    ],
  },
  {
    name: "Saturn",
    radius: 3.5,
    distanceFromSun: 45,
    color: 0xffcc99,
    texture: "public/textures/saturn.jpg",
    orbitSpeed: 0.0000679,
    rotationSpeed: 0.018,
    rings: [
      {
        innerRadius: 4,
        outerRadius: 6,
        texture: "public/textures/saturn_ring.png",
        rotationSpeed: 0.01,
        inclination: {
          x: -Math.PI / 4,
          y: 0,
          z: Math.PI / 8,
        },
      },
    ],
    moons: [
      {
        name: "Titan",
        radius: 0.5,
        distanceFromPlanet: 1.5,
        texture: "public/textures/titan.jpg",
        orbitSpeed: 0.002,
        rotationSpeed: 0.01,
      },
      {
        name: "Rhea",
        radius: 0.4,
        distanceFromPlanet: 2,
        texture: "public/textures/rhea.jpg",
        orbitSpeed: 0.0015,
        rotationSpeed: 0.01,
      },
      {
        name: "Iapetus",
        radius: 0.3,
        distanceFromPlanet: 2.5,
        texture: "public/textures/iapetus.jpg",
        orbitSpeed: 0.0012,
        rotationSpeed: 0.01,
      },
    ],
  },
  {
    name: "Uranus",
    radius: 3,
    distanceFromSun: 55,
    color: 0x66ffff,
    texture: "public/textures/uranus.jpg",
    orbitSpeed: 0.0000238,
    rotationSpeed: 0.015,
    rings: [],
    moons: [
      {
        name: "Titania",
        radius: 0.4,
        distanceFromPlanet: 1.5,
        texture: "public/textures/titania.jpg",
        orbitSpeed: 0.0015,
        rotationSpeed: 0.01,
      },
      {
        name: "Oberon",
        radius: 0.3,
        distanceFromPlanet: 2,
        texture: "public/textures/oberon.jpg",
        orbitSpeed: 0.0012,
        rotationSpeed: 0.01,
      },
    ],
  },
  {
    name: "Neptune",
    radius: 2.8,
    distanceFromSun: 65,
    color: 0x3366ff,
    texture: "public/textures/neptune.jpg",
    orbitSpeed: 0.0000121,
    rotationSpeed: 0.014,
    rings: [],
    moons: [
      {
        name: "Triton",
        radius: 0.5,
        distanceFromPlanet: 1.5,
        texture: "public/textures/triton.jpg",
        orbitSpeed: 0.001,
        rotationSpeed: 0.01,
      },
      {
        name: "Nereid",
        radius: 0.2,
        distanceFromPlanet: 2,
        texture: "public/textures/nereid.jpg",
        orbitSpeed: 0.0008,
        rotationSpeed: 0.01,
      },
    ],
  },
];
